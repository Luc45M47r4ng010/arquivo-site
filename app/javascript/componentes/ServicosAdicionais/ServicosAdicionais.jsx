...
const ServicosAdicionais = () => {
  ...
  const handleSaveServico = () => {
    console.log('Salvando serviço:', newServico);  // Verificando o estado do serviço antes de salvar
    ...
    if (editingServico) {
      ...
      axios.put(`/servicos/${editingServico.id}`, newServico)
        .then(response => {
          console.log('Serviço editado com sucesso:', response.data); // Verificando o que é retornado da API
          if (response.data) { // Garantindo que a resposta tem dados
      ...
      axios.post('/servicos', newServico)
        .then(response => {
  ...
  const handleDeleteServico = (id) => {
    console.log('Excluindo serviço com ID:', id);  // Logando o ID do serviço a ser excluído
    axios.delete(`/servicos/${id}`, { headers: { 'X-CSRF-Token': csrfToken } })
      .then(() => {

...
const PlanPackageSelector = ({ clientId, setPlan, setPackage, setAvailableServices }) => {
  ...
  useEffect(() => {
    if (clientId) {
      axios.get('/planos').then(response => setPlans(response.data));
      axios.get('/pacotes').then(response => setPackages(response.data));
      axios.get('/servicos').then(response => {
  ...
  const loadPackageDetails = async (packageId) => {
    if (packageId) {
      try {
        const response = await axios.get(`/pacotes/${packageId}`);

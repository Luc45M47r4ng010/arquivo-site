...
const FaturamentoCli = () => {
  ...
  const buscarClientes = async () => {
    try {
      const response = await axios.get('http://localhost:3000/clientes'); // Modifique a URL conforme necessário
  ...
  const buscarAssinaturasPorCliente = async (clienteId) => {
    try {
      const response = await axios.get(`http://localhost:3000/assinaturas?cliente_id=${clienteId}`);
